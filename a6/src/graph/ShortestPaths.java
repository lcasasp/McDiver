    ...
    public void singleSourceDistances(Vertex source) {
        ...
        while (!frontier.isEmpty()) {
            ...
            for (Edge edge : graph.outgoingEdges(v)) {
                Vertex destination = graph.dest(edge);
                if (!distances.containsKey(destination)){
                    distances.put(destination, graph.weight(edge) + getDistance(v));
                    frontier.add(destination, getDistance(destination));
                    bestEdges.put(destination, edge);
                }
                else if (graph.weight(edge) + getDistance(v) < getDistance(destination)){
                        distances.put(destination, graph.weight(edge) + getDistance(v));
                        frontier.changePriority(destination, getDistance(destination));
                        bestEdges.put(destination, edge);
                }
            }
        }
